...
public class AuthSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler{
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		// Authentication 객체의 메소드를 이용해서 로그인된 사용자에대한 정보를 얻어낼 수 있다.
		String userName = authentication.getName();
		System.out.println("로그인된 사용자:" + userName);
		// 로그인된 사용자의 이름을 세션에 담아두기
		// 타임리프 페이지에서 ${session.useName }으로 참조할 수 있다.
		...
		if(cashed==null) {
			// 5. 로그인된 환영 페이지로 forward 이동)
			RequestDispatcher rd = request.getRequestDispatcher("/user/login_success");
			rd.forward(request, response);
		}else {
			// 6. 원래 가려던 목적지 경로로 리다이렉트 시킨다(GET방식 요청 파라미터도 자동으로 전달)

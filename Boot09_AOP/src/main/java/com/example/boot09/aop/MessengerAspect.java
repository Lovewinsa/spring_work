...
public class MessengerAspect {
	...
	public void checkGreeting(ProceedingJoinPoint joinPoint) throws Throwable{
		...
		for(Object tmp:args) {
			...
			if(tmp instanceof String) {
				String msg = (String)tmp; // 원래 type으로 casting해 작업한다.
		...
		// 이 메소드를 호출하는 시점에 실제로 aspect가 적용된 메소드가 수행된다.(호출하지 않으면 수행안됨)
		joinPoint.proceed();
	...
	/*
	 * 	return type은 String이고(java.lang 패키지에 있는 데이터type은 패키지명 생략 가능)
	 * 	get으로 시작하는 메소드이고
	 * 	메소드에 전달되는 인자는 없다.
	 * 	java.lang 패키지에 있는 type은 패지키명 생략 가능.
	...
	public Object checkReturn(ProceedingJoinPoint joinPoint) throws Throwable{
		// aspect가 적용된 메소드를 수행하고 리턴디는 데이터 받아오기
		Object obj = joinPoint.proceed();
		// 원래 type으로 casting해서 조사해 볼 수 있다.
		String a = (String)obj;
